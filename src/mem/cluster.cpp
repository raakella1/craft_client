...
MemTransport::delivery MemTransport::plan_delivery(std::chrono::milliseconds delay,
                                                   std::chrono::milliseconds op_timeout) {
    delivery p;
    if (delay.count() <= 0) return p;

    if (op_timeout.count() > 0 && delay >= op_timeout) {
        // The caller is abandoned after `op_timeout`, but the peer still receives the op `delay` from NOW (the
        // late delivery is scheduled at issue). A write therefore lands after writes issued later to undelayed
        // peers, which is what leaves a Missing slot behind.
        return delivery{op_timeout, delay, true};
    }
...
async_result< lsn_pair > MemTransport::send_write(std::shared_ptr< MemCraftReplica > to, client_hdr hdr, int64_t dlsn,
    ...
    if (!rf->up || !rf->write_ok) co_return fail(craft_error::REPLICA_DOWN);
    ...
    if (p.timed_out) {
        ...
        run_after(p.deliver, id, [w = to->weak_from_this(), hdr, dlsn, addr, len, bytes] {
            auto const self = w.lock();
            if (!self) return; // replica gone; the in-flight write dies with it
            auto const* lf = self->fault_snapshot();
            if (!lf->up || !lf->write_ok) return; // went unreachable in flight: dropped
    ...
    auto const* rf2 = to->fault_snapshot();
    if (!rf2->up || !rf2->write_ok) co_return fail(craft_error::REPLICA_DOWN);
...
// ── fault forwarders: read / set the knob on the addressed replica (faults live on the replica now) ──

MemCraftReplica* MemTransport::replica_for(peer_id_t id) const {
    std::lock_guard< std::mutex > g{mu_};
    auto const it = by_id_.find(id);
    return (it == by_id_.end()) ? nullptr : it->second;
}

bool MemTransport::is_up(peer_id_t id) const {
    auto* const r = replica_for(id);
    return r ? r->is_up() : true; // unknown id reads as healthy
}
bool MemTransport::write_allowed(peer_id_t id) const {
    auto* const r = replica_for(id);
    return r ? r->write_allowed() : true;
}
std::chrono::milliseconds MemTransport::delay_for(peer_id_t id) const {
    auto* const r = replica_for(id);
    return r ? r->delay() : std::chrono::milliseconds{0};
}

void MemTransport::set_up(peer_id_t id, bool up) {
    if (auto* const r = replica_for(id)) r->set_up(up);
}
// Writes land ONLY on `keep`: every member not in `keep` drops writes; those in it accept.
void MemTransport::force_subquorum(std::vector< peer_id_t > keep) {
    std::unordered_set< peer_id_t, uuid_hash > const keepset(keep.begin(), keep.end());
    std::vector< replica_endpoint > members_copy;
    {
        std::lock_guard< std::mutex > g{mu_};
        members_copy = members_;
    }
    for (auto const& m : members_copy) {
        if (auto* const r = replica_for(m.id)) r->drop_writes(!keepset.contains(m.id));
    }
}
void MemTransport::clear_faults() {
    ...
    for (auto const& m : members_copy) {
        if (auto* const r = replica_for(m.id)) r->clear_faults();
    }
}
...
void MemTransport::set_delay(peer_id_t id, std::chrono::milliseconds d) {
    if (auto* const r = replica_for(id)) r->set_delay(d);
}
// The client's set-wide deadline (0 = wait forever); NOT a replica fault, so it stays on the transport.
void MemTransport::set_op_timeout(std::chrono::milliseconds t) {
    op_timeout_ms_.store((t.count() > 0) ? t.count() : 0, std::memory_order_release);
}
...
std::vector< MemCraftReplica* > MemTransport::live_replicas_locked() const {
    ...
    for (auto const& m : members_) {
        auto const it = by_id_.find(m.id);
        if (it == by_id_.end()) continue;
        if (!it->second->is_up()) continue; // the replica's own lock-free knob (mu_ held here for by_id_)
...
result< LoginResult > MemTransport::run_login(MemCraftReplica* caller, uint64_t client_token) {
    ...
    {
        std::lock_guard< std::mutex > g{mu_};
        if (!caller->is_up()) return fail(craft_error::REPLICA_DOWN);              // the caller's own lock-free knob
        if (caller->id() != leader_) return LoginResult{{}, -1, 0, 0, 0, leader_}; // redirect: geometry unknown
