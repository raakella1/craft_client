...
// commit frontier, read horizon) the ublk driver drives. It runs against the in-memory reference model via
// make_mem_replica_group -- the client holds the MemCraftReplica backends directly (craft_replica), NO
// volume_handle -- so this TU is HomeStore-free and links WITHOUT libhomeblocks (like test_craft_memory). No
// ublk device, no iomgr. n=1 is the stepping stone; n=3 (with force_subquorum) exercises the quorum path.
...
struct Cluster {
    craft::MemReplicaGroup set;
    volume_id_t vid;
    std::shared_ptr< craft::craft_client > client;
};

// Stand up an n-replica in-memory set + a client over it (the client co-owns the replica backends, so `set`
...
Cluster make_cluster(uint32_t n, uint32_t leader = 0) {
    ...
    auto set = craft::make_mem_replica_group(vid, n, PAGE);
    // The client holds the replica BACKENDS directly (craft_replica), not volume_handles -- backend-agnostic
    // and HomeStore-free. `set` co-owns the same replicas (+ net) for fault injection.
    std::vector< std::shared_ptr< craft::craft_replica > > backends(set.replicas.begin(), set.replicas.end());
    auto client = std::make_shared< craft::craft_client >(std::move(backends), leader);
...
TEST(CraftClient, RecoveryLoginReadsDurableDataNotZeros) {
    ...
    std::vector< std::shared_ptr< craft::craft_replica > > fresh_backends(cl.set.replicas.begin(),
                                                                          cl.set.replicas.end());
    auto fresh = std::make_shared< craft::craft_client >(std::move(fresh_backends));
