...
TEST(CraftMemModel, DelayBelowTimeoutMerelySlowsTheOp) {
    ...
    auto [term, dl] = login_ok(g);
    auto& r = *g.replicas[0];
    g.net->set_op_timeout(std::chrono::milliseconds{500}); // op_timeout is the client deadline: stays on net
    r.set_delay(std::chrono::milliseconds{20});            // latency is the replica's own knob now
