...
Status: implemented. This document specifies the on-wire encoding the `craft_client` transport
(`craft_wire` + `net/`) is built against -- the concrete encoding of the client-facing verbs and the
transport requirements enumerated in [`client-internals.md`](client-internals.md) ("What the transport must
provide"). The reference model (`craft_reference`) can also drive the same client in-process over `MemTransport`.
...
- **`HELO`** binds an *additional* connection (another queue, or a follower) to the already-established
...
### HELO (request op = 3, response op = 4)
...
(`craft/types.hpp`), so client-side mapping is a cast.
